...
struct CalculatorBrain {
    var accumulator: ( value: Double?, description: String) = ( 0, "0")  
    mutating func reset () {
        setOperand(0)
        ...
        if let operation = operations[symbol] {
            ...
            case .constant(let value):
                ...
                if pendingBinaryOperation != nil  {
                    accumulator.description = pendingBinaryOperation!.description + symbol
                    pendingBinaryOperation!.description = String ()
                }
            case .equals:
                performPendingBinaryOperation() // к выполнению бинарной операции
            case .unaryOperation(let function):
                if accumulator.value != nil {
                     var relation = symbol + "(" + accumulator.description + ")"
                    if pendingBinaryOperation != nil { // есть отложенная бинарная операция? 7+√(9)
                        relation = pendingBinaryOperation!.description + relation // добавить описание первого операнда и отложенной операции
